...
const TeachersList = () => {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-4xl font-bold mb-6 text-center">{t('listt')}</h1>
        ...
        {teachers.map(teacher => (
          <Link to={`/teacher/${teacher.id}`} key={teacher.id}>
            <div className="bg-white shadow-md rounded-lg p-6 hover:shadow-xl transition-shadow duration-300">
              ...
              <p className="text-gray-600"> {t('level')}{teacher.level}</p>
              <p className="text-gray-600">{t('group')}: {teacher.groupcount}</p>
            </div>
          </Link>
        ))}
      </div>
